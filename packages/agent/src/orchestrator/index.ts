...
type ProgressReporter = (update: AgentProgressUpdate) => void;
...
export class AgentOrchestrator {
  ...
  private emitProgress(
    ...
    try {
      ...
      this.logger.warn({ error: String(error), stage, message }, 'Progress callback failed');
    }
  }
  ...
  async run(options: RunOptions): Promise<AgentRunResult> {
    const {
      ...
      scope,
      query,
      includeDebug = this.config.debug.enabled,
      onProgress,
    } = options;
    ...
    this.emitProgress(onProgress, 'starting', 'Starting analysis');

    // Validate scope
    if (scope.rangeEnabled && !scope.range) {
      this.emitProgress(onProgress, 'error', 'Commit range is enabled but not specified');
    ...
    try {
      ...
      const result = await withTimeout(
        this.executeAgentLoop(runId, scope, query, toolContext, onProgress),
      ...
      this.emitProgress(onProgress, 'completed', 'Finished analysis');
  ...
  private async executeAgentLoop(
    ...
    if (!this.modelRouter.isAvailable()) {
      this.emitProgress(onProgress, 'error', 'LLM is not available');
      return {
    ...
    if (scope.rangeEnabled && scope.range) {
      this.emitProgress(onProgress, 'listing_commits', 'Looking through commits in the selected range');
      ...
      if (!commitsResult.success || !commitsResult.commits) {
        this.emitProgress(
          onProgress,
          'error',
          `Failed to retrieve commits: ${commitsResult.error ?? 'Unknown error'}`
        );
        return {
      ...
      this.emitProgress(
        onProgress,
        'listing_commits',
        `Loaded ${retrieved.commits.length} commits`
      );
      this.logger.debug({ count: retrieved.commits.length }, 'Retrieved commits');
    }

    if (retrieved.commits.length === 0) {
      this.emitProgress(onProgress, 'completed', 'No commits found in the specified range');
      return {
    ...
    try {
      this.emitProgress(
        onProgress,
        'exploring',
        'Exploring commits to find changes relevant to your question'
      );
      ...
      this.emitProgress(
        onProgress,
        'error',
        `Exploration had an issue: ${error instanceof Error ? error.message : String(error)}`
      );
    ...
    if (relevantCommits.size > 0) {
      this.emitProgress(onProgress, 'ranking', 'Ranking relevant commits');
      ...
      try {
        ...
        this.emitProgress(onProgress, 'ranking', 'Ranking failed; using fallback ordering');
    ...
    if (topK.length > 0) {
      this.emitProgress(onProgress, 'fetching_evidence', 'Collecting details and diffs for top commits');
      await this.fetchDetailsAndDiffs(toolContext, topK, retrieved, onProgress);
    }

    // Step 5: Synthesize final answer
    this.emitProgress(onProgress, 'synthesizing', 'Writing final answer');
  ...
  private async runAgentExplorationLoop(
    ...
    while (iteration < maxIterations) {
      ...
      if (!result.toolCalls || result.toolCalls.length === 0) {
        ...
        this.emitProgress(onProgress, 'exploring', 'Finished commit exploration');
        break;
      }
  ...
  private async executeToolCall(
    ...
    try {
      ...
      this.emitProgress(onProgress, 'tool_call', this.describeToolCall(name, args, retrieved.commits));
      ...
      switch (name) {
        ...
        case 'mark_relevant_commit': {
          ...
          this.emitProgress(onProgress, 'tool_call', `Marked commit ${sha.slice(0, 8)} as relevant`);
    ...
    switch (toolName) {
      case 'search_commits': {
        ...
        if (keywords.length === 0) {
          return 'Looking through commits for matching keywords';
        }
        const compactKeywords = keywords.slice(0, 4).join(', ');
        return `Looking through commits for: ${compactKeywords}`;
      }
      case 'get_commit_details': {
        const requestedSha = typeof args.sha === 'string' ? args.sha : '';
        if (!requestedSha) {
          return 'Reading commit details';
        }
        const expandedSha = this.expandSha(requestedSha, commits);
        return `Reading commit ${expandedSha.slice(0, 8)} details`;
      }
      case 'get_diff': {
        const requestedSha = typeof args.sha === 'string' ? args.sha : '';
        if (!requestedSha) {
          return 'Reading commit diff';
        }
        const expandedSha = this.expandSha(requestedSha, commits);
        return `Reading commit ${expandedSha.slice(0, 8)} diff`;
      }
      case 'mark_relevant_commit': {
        const requestedSha = typeof args.sha === 'string' ? args.sha : '';
        if (!requestedSha) {
          return 'Saving relevant commit';
        }
        const expandedSha = this.expandSha(requestedSha, commits);
        return `Saving commit ${expandedSha.slice(0, 8)} as relevant`;
      }
      default:
  ...
  private async fetchDetailsAndDiffs(
    ...
    for (const candidate of candidates) {
      ...
      if (!retrieved.details.has(candidate.sha)) {
        this.emitProgress(onProgress, 'fetching_evidence', `Reading commit ${candidate.sha.slice(0, 8)} details`);
      ...
      if (!retrieved.diffs.has(candidate.sha)) {
        this.emitProgress(onProgress, 'fetching_evidence', `Reading commit ${candidate.sha.slice(0, 8)} diff`);
